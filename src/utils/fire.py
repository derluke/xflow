...
class FIRE(dr.Project):
    ...
    def main_feature_reduction(
        ...
        restart_autopilot: boolean, if True will restart current setup of autopilot after each iteration. Always True
        for 'Rank Aggregation' method. Default False
        lifes: int, stopping criteria, if no best model produced after lifes iterations, stop feature reduction.
        Default 3
        ...
        partition: str, whether to use 'validation' or 'crossValidation' partition to get the best model on. Default
        'validation'
        main_scoring_metric: str, DR metric to check performance against, Default None
        initial_impact_reduction_ratio: float, ratio of total feature impact that new feature list will contain.
        Default 0.95
        remove_negative_only_first: boolean, if True will drop only those features with 0 or negative FI per iteration.
        Default False
        best_model_search_params: dict, dictonary of parameters to search the best model. See official DR python
        api docs.
        Default None
        ...
        while lifes >= 0 and max_iterations > 0:
            ...
            if reduction_method in ["Simple", "DR Reduced"]:
                ...
                if reduction_method == "Simple":
                    new_featurelist_name = "M{}_top{}_r{}_iter{}".format(
                    ...
                    new_featurelist_name = "DR Reduced M{}_r{}".format(
                        best_model.model_number, int(ratio * 100)
                    )
